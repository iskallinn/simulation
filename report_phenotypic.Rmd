  ...
  summaryBy(
    ...
    skin.qual.var+
      ...
      avg.skin.length.male+avg.skin.length.female+feed.per.skin+skin.price
        +pr.farm.margin+feeding.cost+avg.skin.price
...
Note that this is only the feeding costs from july to november. It is currently missing the feeding costs of the breeding stock, the feed consumed by females during lactation and feed consumed by kits in june

```{r, echo= FALSE}
qplot(Gen, feeding.cost.mean, data = summarized) + geom_errorbar(
  aes(
    x = Gen,
    ymin = feeding.cost.mean - sqrt(feeding.cost.var),
    ymax = feeding.cost.mean + sqrt(feeding.cost.var)
  ),
  width = 0.25,
  data = summarized
) + geom_line(linetype =2)+
...
qplot(Gen, avg.skin.price.mean, data=summarized) + geom_line(linetype =2) +geom_errorbar(
  ...
  labs(title ="Average skin price", x="Simulation Year", y="Feed costs per skin [DKK]")
```
...
```{r,echo=FALSE}
qplot(Gen, pr.farm.margin.mean, data=summarized) + geom_line(linetype =2) +geom_errorbar(
  aes(
    x = Gen,
    ymin = pr.farm.margin.mean - sqrt(pr.farm.margin.var),
    ymax = pr.farm.margin.mean + sqrt(pr.farm.margin.var)
  ),
  width = 0.25,
  data = summarized
)+
